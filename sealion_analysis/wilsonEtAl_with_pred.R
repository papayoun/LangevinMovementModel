...
##################################
# Fit CTMC UD to all animals    ##
##################################
allpred <- NULL

for(id in unique(ssl_data$Deploy_ID)) {
    cat("\nProcessing individual",id,"...\n")
    temp_dat <- dplyr::filter(ssl_data, Deploy_ID==id) %>% arrange(GMT) #pull out one sea lion
    ...
    pred <- data.frame(ID = predloc$Deploy_ID, 
...
write.csv(allpred, file="~/git/LangevinMovementModel/sealion_analysis/SSLpreddat.csv",
          row.names=FALSE)
